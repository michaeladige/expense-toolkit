...
- `workdays.ts` — `isWorkingDay` (Mon-Fri minus a holiday set) plus `firstWorkingDay`/`lastWorkingDay`, which return `null` for a period with no working day at all (reachable for a week, not a month). The weekend is deliberately fixed to Sat/Sun rather than derived from the holiday country: providers don't publish weekend days.
...
**Holidays** (`src/hooks/useHolidays.ts`): resolves the working-day anchors above. Modeled on `useExchangeRates` (provider array, localStorage cache, status union) with three deliberate differences: `HolidayStatus` adds `off` for "no country configured", without which the gate below would wait forever on a fetch that never starts; there's **no age-based expiry** (published holidays don't drift like FX — validity is "does the cache cover this country and these years"); and the returned `days` Set is **memoized**, because it feeds `useRecurring`'s effect deps and a fresh Set per render would re-run a state-writing check every render. Nager returns `Observance` entries that are *not* days off — only `types` containing `"Public"` counts. `knownYears` exists so callers can tell "no holidays" from "not loaded"; a rule dormant past the fetched window resolves against weekends and the UI marks it approximate.
